...
require("thumb.php");
 ...
 $mysqli = new mysqli($mysql_server_name, $mysql_username, $mysql_password, $mysql_database);  
  if (!$mysqli){
	    $output = array('data'=>'', 'msg'=>"连接失败".mysqli_connect_error($mysqli),'success'=>'0');
        die(JSON($output));
  }
...
if (empty($a)) {
 ...
 // 检查 token 
 $ignoreList = array('login');
	 if (!in_array($a, $ignoreList)) {
	  $tokenRes = checkToken();
		  if ($tokenRes['status'] == '1') {
			exit(JSON(array('msg'=>$tokenRes['msg'], 'status'=>'999')));
		  }
}
	...
	if ($a == 'apitest') {
		$flat = $_GET['flat'];
		if ($flat == '1') {
			$output = array('data'=>'', 'msg'=>'成功','success'=>'1');
		} else {
 ...
 if ($a == 'login') {
	...
	if (mysqli_num_rows($result) > 0) {  //是否是管理员
		$items = array();
   ...
   //取得公共传输的公密钥
	...
	if (mysqli_num_rows($result2) > 0) {
		...
		$sql = "update team_login set logintime=now(),token_key='$uniqid' where userno='$uid'";
		if (!$mysqli->query($sql)) {
			$output = array('data' => '', 'msg' => '登录处理错误', 'success' => '0');
			exit(JSON($output));
		}	
	} else {
		$pubkey = getRandChar(64);
		$sql = "insert DELAYED team_login set userno='$uid',pubkey='$pubkey',logintime=now(),token_key='$uniqid',serialno='$macid',ver='$v'";
		if (!$mysqli->query($sql)) {
			$output = array('data' => '', 'msg' => '登录处理错误', 'success' => '0');
			exit(JSON($output));
		}			
	...
	//登录验证
	$query5 = "SELECT * FROM team_salesman  where (userno='$uid') and ((password='$mrepass') OR (repassword='$mrepass' AND (lastdate BETWEEN DATE_ADD( NOW(), INTERVAL -1 HOUR)  AND  NOW() )))";	
    ...
    if (mysqli_num_rows($result) > 0){
    	...
    	if ($row['team']>0){		//管理员身份，直接登录
			$outdata=array('username'=>$row['username'],'userno'=>$row['userno'],'address'=>$row['address'],'phone'=>$row['phone'],
					  ...
					  'admin'=>$admin,'division'=>$division,'pubkey'=>$pubkey,'token'=>$uniqid);
		} else {
			 $query2 = "SELECT admin,isteam FROM `teams` WHERE userno='$uid' AND (division=admin) AND division!='' and astatus='1'";	
 ...
 if ($a == 'registeruser') {
	   ...
	   $smstext = "";     
		...
		$result = $mysqli->query($query);
		 if (mysqli_num_rows($result)>0){		 //存在userno		    
   ...
   if ($a == 'edituser') {
    ...
    //更改密码
 	if ($a == 'setpass') {
		...
		$mrepass = setstrmd5(md5($pass));  // md5加密
		$mnewpass = setstrmd5(md5($newpass));  // md5加密
		
		$query = "select * from team_salesman where (userno='$uid') and ((password='$mrepass') or (repassword = '$mrepass')) ";
		$result = $mysqli->query($query);
		if (mysqli_num_rows($result) <= 0){		
 ...
 if ($a == 'getpana') {
	...
	if($mkey=='-1'){   //按billdate 排序
		if($search=='0'){  //非搜索状态   
			 $query = "select *,'' AS mkey  from `team_customer` where (admin='$admin' and (saler='$saler')) and ispana=1 and (`status`>-1) order by billdate  desc LIMIT $page,15";
		}else if($search=='1'){
		     $query = "select *,'' AS mkey  from `team_customer` where (admin='$admin' and (saler='$saler' or `status`='1')) and ispana=1 and (`status`>-1) and  title like '%$keyword%' order by billdate  desc LIMIT $page,15";
	  }	
	}else if($mkey=='0'){           //按名称排序
	    if($search=='0'){  //非搜索状态  
		    $query = "select *,'' AS mkey  from `team_customer` where (admin='$admin' and (saler='$saler')) and ispana=1 and (`status`>-1) order by title ASC LIMIT $page,15";
		}else if($search=='1'){
			$query = "select *,'' AS mkey from `team_customer` where (admin='$admin' and (saler='$saler' or `status`='1')) and ispana=1 and (`status`>-1) and title like '%$keyword%' order by title ASC LIMIT $page,15";
		}	
	}else if($mkey=='1'){     //客访时间，最新开头 
	     if($search=='0'){   //非搜索状态下
			$query="SELECT a.*,'' AS mkey FROM `team_customer` a LEFT JOIN `team_visit` b ON (a.title=b.customername)  
 ...
 if ($a == 'getcheckcontract') {
    ...
    $obj = json_decode($input, true);
	 ...
	 $checker = gettostr($obj['checker']);			
		...
		if (!$mysqli->query($sql))
		{       
			$output = array('data'=>'', 'msg'=>'合同保存失败','success'=>'0');
			exit(JSON($output));
		}	
...
if ($a == 'updatecontract') {		//更改合同
      ...
      if($orderflag=='0'){  		//保存数据	及  提交审核
		...
		if (!$mysqli->query($sql))
		{       
			$output = array('data'=>'', 'msg'=>'合同添加失败','success'=>'0');
			exit(JSON($output));
		}	
...
if ($a == 'editcontract') {		//添加合同订单
 ...
 if ($a == 'addteamuser') {
    ...
    if (!$mysqli->query($sql))
    {       
        $output = array('data'=>'', 'msg'=>'添加队员错误！','success'=>'0');
        exit(JSON($output));
    }
 ...
 if ($a == 'getmygroupinfo') {
  ...
  if ($a == 'addcustomer') {
    ...
    if ($uid == '')  {
        $output = array('data'=>$uid, 'msg'=>gettostr('uid is not none'),'success'=>'0');
        exit(JSON($output));
    }
    if ($title == '')  {
...
function getCount_days($date1,$date2){
  ...
  //新增客户 查询是否名称重复
   if ($a == 'selectcusttitle') {
	...
	$result = $mysqli->query($query);
	if (mysqli_num_rows($result) > 0) {  //存在记录时，又比较客访时间天数差是否小于30天
		$items = array();
		while ($row = $result->fetch_assoc()) {
		array_push($items, $row);	
		$refreshdate=$row['refreshdate']; //最近客访时间
  ...
  if ($a == 'getproducts') {
	 ...
	 if($mkey=='-1'){   //按billdate 排序
		 if($search=='0'){   //非搜索状态下
	       ...
	       $query="SELECT a.*,'' AS mkey FROM `team_customer` a LEFT JOIN `team_visit` b ON (a.title=b.customername)  WHERE (a.admin='$admin' and (a.saler='$uid' OR a.`status`='1')) and (a.ispana=0) and (a.`status`>-1) and (a.typeno='$sid') and title like '%$keyword%' GROUP BY b.customername  ORDER BY b.billdate DESC LIMIT $page,15";		   
...
if ($a == 'updateworks') {
  ...
  $msg = $flat == '0' ? '签到' : '签退';
	...
	// 先查询是否今天创建了数据
	$query = "SELECT * FROM `team_attendance` WHERE userno='$uid' and admin='$admin' AND billdate BETWEEN '$starttime' AND '$endtime' LIMIT 1";
	$result = $mysqli->query($query);
  $row = $result->fetch_assoc();
  ...
  if ($row) {
    ...
    if ($flat == '0') {
      // 签到只能签一次
      if ($row['startcheckwork'] == '1') {
        $output = array('data'=>'', 'msg'=>'您今天已经签到了', 'success'=>'0');
        $mysqli->close(); 
        exit(JSON($output)); 
      }
      $sql = "UPDATE `team_attendance` SET startcheckwork='1',startworktime=NOW(),startworkaddress='$address',startworklatitude='$lat',startworklongitude='$log' WHERE userno='$uid' AND id='$id'";
    } else {
      $msg .= '更新';
      $sql = "UPDATE `team_attendance` SET endcheckwork='1',endworktime=NOW(),endworkaddress='$address',endworklatitude='$lat',endworklongitude='$log' WHERE userno='$uid' AND id='$id'";       
    }

    if(!$mysqli->query($sql)) {       
      $output = array('data'=>'', 'msg'=>$msg.'失败','success'=>'0');
    } else {
      $output = array('data'=>'', 'msg'=>$msg.'成功','success'=>'1');
    }
  } else {
    if ($flat == 0) {
 ...
 if ($a == 'selectcont') {
    ...
    $obj = json_decode($input, true);
	 ...
	 $image3 = gettostr($obj['image3']);
		...
		if (!$mysqli->query($sql))
		{       
			$output = array('data'=>'', 'msg'=>'保存失败','success'=>'0');
			exit(JSON($output));
		}	
...
if ($a == 'sendmsg') {
 ...
 if ($a == 'putwaresadd') {
  ...
  if ($a == 'singlewareinfo') {
    $billno = $_GET['billno'];
    if (!$billno) {
      $output = array('data'=>'', 'msg'=>'param billno error','success'=>'0');
      exit(JSON($output));
    }

    $query = "SELECT * FROM `team_wares` WHERE billno='$billno' AND admin='$admin' AND `status`>'-1'"; 
    $result = $mysqli->query($query);

    $row = $result->fetch_assoc();
    if ($row) {
      $output = array('data'=>$row, 'msg'=>'查询成功','success'=>'1');
    } else {
      $output = array('data'=>'', 'msg'=>'未能在库存中查找到该商品','success'=>'0');
    }
    exit(JSON($output));
  }
 ...
 if ($a == 'putstockin') {
    ...
    $obj = json_decode($input, true);
	 ...
	 $kk = gettostr($obj['kk']); // if判断操作 0进1出
	 $qtyflat = gettostr($obj['qtyflat']); // 商品的类型 qty=>成品 qty1=>次品 qty2=>坏品 qty3=>其它
   
   if ($qtyflat !== 'qty' && $qtyflat !== 'qty1' && $qtyflat !== 'qty2' && $qtyflat !== 'qty3') {
   		$qtyflat = 'qty';
   }
   ...
   if($kk=='0'){
	    $sql = "INSERT INTO `team_stockin` SET billno='$billno',billdate=NOW(),customerno='$customerno',customername='$customername',username='$username',admin='$admin',wareno='$mbillno',warename='$name',model='$type',
		  productno='$number',price='$price',`description`='$describe',$qtyflat='$knum',unit='$unit',image1='$image1',image2='$image2',image3='$image3',serialno='$serialno'";
   }else if($kk=='1'){
  ...
  if ($a == 'delstock') {
  	...
  	if ($mkey == '0') {
  		$tablename = 'team_stockin';
  	} else {
  		$tablename = 'team_stockout';
  	}

  	$sql = "DELETE FROM $tablename WHERE billno='$billno'";
  	if (!$mysqli->query($sql)) {
  		$output = array('data'=>'', 'msg'=>'删除失败','success'=>'0');
  	} else {
  		$output = array('data'=>'', 'msg'=>'删除成功','success'=>'1');
  	}
  	exit(JSON($output));
  }
  ...
  if ($a == 'restock') {
  	...
  	$billno = $_GET['billno'];
  	$knum = $_GET['knum']; // 数量
  	$qtyflat = $_GET['qtyflat']; // 当前选择的商品类型 qty=>成品 qty1=>次品 qty2=>坏品 qty3=>其它
  	$beforeqtyflat = $_GET['beforeqtyflat']; // 之前选择的商品类型 qty=>成品 qty1=>次品 qty2=>坏品 qty3=>其它
  	...
  	$sql = "UPDATE $tablename SET $beforeqtyflat=0, $qtyflat='$knum' WHERE billno='$billno'";
  	if (!$mysqli->query($sql)) {
  		$output = array('data'=>'', 'msg'=>'修改失败','success'=>'0');
  	} else {
  		$output = array('data'=>'', 'msg'=>'修改成功','success'=>'1');
 ...
 if ($a == 'getallwarerecords') {
    ...
    if (($customerno == '') ) {
         if($key=='0'){  //进货	 
	     $query="SELECT DATE_FORMAT(billdate,'%Y-%m-%d') AS billdate, (qty+qty1+qty2+qty3)*price AS amount,price,qty,qty1,qty2,qty3 FROM `team_stockin` WHERE  admin='$admin' GROUP BY DATE_FORMAT(billdate,'%Y-%m-%d') ORDER BY DATE_FORMAT(billdate,'%Y-%m-%d') DESC ";
	  }else{       //出货
	      $query="SELECT  DATE_FORMAT(billdate,'%Y-%m-%d') AS billdate, (qty+qty1+qty2+qty3)*price AS amount,price,qty,qty1,qty2,qty3 FROM `team_stockout` WHERE  admin='$admin' GROUP BY DATE_FORMAT(billdate,'%Y-%m-%d') ORDER BY DATE_FORMAT(billdate,'%Y-%m-%d') DESC ";
	  }
    }else{
      if($key=='0'){  //进货	 
	     $query="SELECT DATE_FORMAT(billdate,'%Y-%m-%d') AS billdate, (qty+qty1+qty2+qty3)*price AS amount,price,qty,qty1,qty2,qty3 FROM `team_stockin` WHERE customerno='$customerno' AND admin='$admin' GROUP BY DATE_FORMAT(billdate,'%Y-%m-%d') ORDER BY DATE_FORMAT(billdate,'%Y-%m-%d') DESC ";
	 
	  }else{       //出货
	  $query="SELECT DATE_FORMAT(billdate,'%Y-%m-%d') AS billdate, (qty+qty1+qty2+qty3)*price AS amount,price,qty,qty1,qty2,qty3 FROM `team_stockout` WHERE customerno='$customerno' AND admin='$admin' GROUP BY DATE_FORMAT(billdate,'%Y-%m-%d') ORDER BY DATE_FORMAT(billdate,'%Y-%m-%d') DESC ";
	  }
	}
      $result = $mysqli->query($query);
      $output = array();
...
if ($a == 'getwages') {
	...
	if ($a == 'delwage') {
		$billno = $_GET['billno'];
		if (!$billno) {
			$output = array('data' => '', 'msg' => '参数验证error', 'success' => '0');
      exit(JSON($output));
		}
		$sql = "DELETE FROM `team_wage` WHERE billno='$billno'";
		if (!$mysqli->query($sql)) {
      $output = array('data' => '', 'msg' => '删除失败', 'success' => '0');
    } else {
    	$output = array('data' => '', 'msg' => '删除成功', 'success' => '1');
 ...
 if ($a == 'putwage') {
  ...
  //我的团队测试
   if ($a == 'getmyteam') {
	...
	if($key=='-1'){  //副管理员
		$sql="SELECT B.id,B.userno,A.num,C.hname,C.image,B.team,B.teamdate,B.teamname FROM (SELECT COUNT(*) AS num FROM `teams` WHERE admin='$admin') A,
             (SELECT id,userno,team,teamdate,teamname FROM `team_salesman` WHERE userno='$admin') B,
		...
		$sql="SELECT B.id,B.userno,A.num,B.image,B.hname,B.team,B.teamdate,B.teamname FROM (SELECT COUNT(*) AS num FROM `teams` WHERE admin='$uid') A,
             (SELECT id,userno,image,team,teamdate,username AS hname,teamname FROM `team_salesman` WHERE userno='$uid') B";
	}else if($key=='1'){ //(组长角度) 已有队员num, 组长名username, 工作部门 teamname		
		$sql="SELECT B.id,B.userno,A.num,B.image,B.hname,B.teamname,C.team,B.teamdate FROM (SELECT COUNT(*) AS num FROM `teams` WHERE division='$uid') A,
             (SELECT id,userno,image,teamname,teamdate,username AS hname FROM `team_salesman` WHERE userno='$uid') B,
		...
		$sql="SELECT B.id,B.userno,A.num,B.image,B.hname,B.teamname,B.team,B.teamdate FROM (SELECT COUNT(*) AS num FROM `teams` WHERE division ='$division') A,
             (SELECT id,userno,image,teamname,team,teamdate,username AS hname FROM `team_salesman` WHERE userno='$division') B";	
	}
	...
	if (mysqli_num_rows($result) > 0) {  
        ...
        $htdate= $row['teamdate'];  
        $htname= $row['teamname']; 	
		$mrow = array('id'=>$id,'userno'=>$userno,'image'=>$image,'hnum'=>$hnum,'hname'=>$hname,'htnum'=>$htnum,'htdate'=>$htdate,'htname'=>$htname);
        array_push($mitems, $mrow);		
		}		
	  }		
	if($key=='-1'){//副管理员查询有多少个组长(副管理员不显示包括自己的其它副管理员)
		$query = "SELECT a.id,a.billno,a.isteam,a.astatus,b.username,b.userno,b.image,b.jobnumber,b.teamname,b.job FROM teams AS a LEFT JOIN `team_salesman` AS b ON a.`userno`=b.`userno` 
		...
		$query = "SELECT a.id,a.billno,a.isteam,a.astatus,b.username,b.userno,b.image,b.jobnumber,b.job,b.teamname FROM teams AS a LEFT JOIN `team_salesman` AS b ON a.`userno`=b.`userno` 
      ...
      $items = array();
     while($row=$result->fetch_assoc()){
        $rows = array('id'=>$row['id'],'billno'=>$row['billno'],'username'=>$row['username'],'userno'=>$row['userno'],'image'=>$row['image'],'jobnumber'=>$row['jobnumber'],'teamname'=>$row['teamname'],
 ...
 if ($a == 'outteam') {
    ...
    if (($userno == '') ) {
	 ...
	 if (($password == '') ) {
        $output = array('data'=>'', 'msg'=>'password不能为空','success'=>'0');
        exit(JSON($output));
    }
	
	$mrepass = setstrmd5(md5($password));  // md5加密
	
    $query = "SELECT username FROM team_salesman  where (userno='$userno') and ((password='$mrepass') OR (repassword='$mrepass' AND (lastdate BETWEEN DATE_ADD( NOW(), INTERVAL -1 HOUR)  AND  NOW() )))";	
 ...
 if ($a == 'getcrudelist') {
  ...
  if ($a == 'setcrude') {
	...
	$billno= @gettostr($obj['billno']) ? gettostr($obj['billno']): substr(date("ymdHis"),1,11).mt_rand(100,999);	
 ...
 if ($a == 'getartlist') {
     ...
     if (($admin == '') ) {
	  ...
	  $query = "SELECT * FROM `team_art` WHERE admin='$admin' ";
	  if ($customer<>''){
		  $query = $query. "  and custname = '$customer'";
	  }
 ...
 if ($a == 'getartlist') {
     ...
     if (($admin == '') ) {
	  ...
	  $query = "SELECT * FROM `cloth_art` WHERE admin='$admin' ";
	  if ($customer<>''){
		  $query = $query. "  and custname = '$customer'";
	  }
 ...
 if ($a == 'getworker') {
	 $admin=$_GET['admin'];			//管理员编号
	 $flag=$_GET['flag'];	//0 取得收胚人员   1 取得司机   100 取得全部用户
	...
	if ($flag==0){
		$query = "SELECT * FROM `cloth_worker` WHERE admin='$admin' and wtype='收胚员'";
	} else if ($flag==1){
		$query = "SELECT * FROM `cloth_worker` WHERE admin='$admin' and wtype='司机'";
	} else if($flag==100){
		$query = "SELECT * FROM `cloth_worker` WHERE admin='$admin'";
	} 
 ...
 if ($a == 'getcarplate') {
	...
	$query = "SELECT * FROM `cloth_plate` WHERE admin='$admin'";
...
function checkToken() {
  ...
  if (!$token) {
    return array('status'=>'1', 'msg'=>'token empty');
  }
  $query = "SELECT * FROM team_login WHERE token_key='$token' LIMIT 1";
  $result = $mysqli->query($query);
  $data = $result->fetch_assoc();
  if (!$data) {
    return array('status'=>'1', 'msg'=>'token error');
  }
