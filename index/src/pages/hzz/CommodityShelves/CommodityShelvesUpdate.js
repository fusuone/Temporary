...
import { Card, Divider, Menu, Dropdown, Icon, Spin, Button, Modal, Select, message, Input, Row, Col, DatePicker, Form, Radio, Upload } from 'antd';
...
@Form.create()
class CommodityShelvesUpdate extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  setFieldsValue = (props) => {
    ...
    this.state.admin=activeItem.admin//用户
    this.state.wareno=activeItem.wareno//编号
    this.state.waretype=activeItem.waretype//商品类型
    this.state.ticket=activeItem.ticket//可用卷
    this.state.envelope=activeItem.envelope//可用红包
    this.state.image1=activeItem.image1//头像
    this.state.makedate=activeItem.makedate//生产日期
    this.props.form.setFieldsValue({ 
        datat:this.state.makedate!="0000-00-00"?moment(this.state.makedate):moment("2000-01-01"),
        waresname:activeItem.waresname,
        billno:activeItem.billno,
        model:activeItem.model,
        productno:activeItem.productno,
        price:activeItem.price,
        waretype:activeItem.waretype,
        unit:activeItem.unit,
        place:activeItem.place,
        warranty:activeItem.warranty,
        buylimit:activeItem.buylimit,
        envelope:activeItem.envelope,
...
//确定触发
handleSubmit =(e)=>{
  var waresname,admincode,billno,onsale,model,productno,price,unit,waretype,place,makedate,warranty,buylimit,envelope,ticket,net,describe,image1
  ...
  this.props.form.validateFields((err, values) => {
    if(describe==""||net==""||warranty==""||place==""){
              ...
              <Col>
                <Form.Item label="型号">
                  {getFieldDecorator('model')(
                    <Input  readonly="readonly" placeholder="暂无型号" id="model" name="model" type="text"/>
                  )}
                </Form.Item>
              </Col>
              <Col>
              ...
              <Col>
                <Form.Item label="是否可用卷">
                  {getFieldDecorator('ticket', {
                    rules: [{ required: true, message: '请选择是否可用卷' }],
                  })(
                    <Select
...
export default CommodityShelvesUpdate;